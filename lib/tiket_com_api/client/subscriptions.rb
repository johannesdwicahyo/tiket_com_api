...
module TiketComApi
  class Client
    ...
    module Subscriptions
      ...
      #     TiketComApi.create_subscription("user", "http://example.com/instagram/callback")
      # @format :json
      # @authenticated true
      #
      #   Requires client_secret to be set on the client or passed in options
      ...
      #     TiketComApi.delete_subscription(:object => "user")
      # @format :json
      # @authenticated true
      #
      #   Requires client_secret to be set on the client or passed in options
      ...
      # As a security measure (to prevent DDoS attacks), TiketComApi sends a verification request to your server
      # after you request a subscription.
      # This method parses the challenge params and makes sure the call is legitimate.
      #
      # @param params the request parameters sent by TiketComApi.  (You can pass in a Rails params hash.)
      ...
      #                     will receive the verify_token received back from TiketComApi.
      #
      # @return the challenge string to be sent back to TiketComApi, or false if the request is invalid.
      def meet_challenge(params, verify_token = nil, &verification_block)
        if params["hub.mode"] == "subscribe" &&
            ...
            # you can pass in a block, which we'll call with the verify_token sent by TiketComApi
      ...
      # Public: As a security measure, all updates from TiketComApi are signed using
      ...
      #   # in Rails controller
      #   def receive_update
      #     if TiketComApi.validate_update(request.body, headers)
      #       ...
      #     else
      ...
      #   @param json [String] The JSON response received by the TiketComApi real-time server
      ...
      #     TiketComApi.process_subscription(params[:body]) do |handler|
      #
      #       handler.on_user_changed do |user_id, data|
      #
      #         user = User.by_instagram_id(user_id)
      #         @client = TiketComApi.client(:access_token => _access_token_for_user(user))
      ...
      def process_subscription(json, options={}, &block)
        ...
        if options.has_key?(:signature)
          ...
          if options[:signature] != verify_signature
            raise TiketComApi::InvalidSignature, "invalid X-Hub-Signature does not match verify signature against client_secret"
          end
        end
